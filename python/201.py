...
def max_weight_path(lst):
    ...
    weight = [[0]*(i+1) for i in range(n)]      # max accumulative weight at each node initialized
    for i in range(n):                          # go through each leval
        for j in range(i + 1):                  # go through each node: level i has i+1 nodes
            if j == 0:          # leftmost node: its parent is previous level's leftmost node
                weight[i][j] = lst[i][j] + weight[i-1][j]
            elif j == i:        # rightmost node: its parent is previous level's rightmost node
                weight[i][j] = lst[i][j] + weight[i-1][j-1]
            else:               # in-between nodes: it has 2 parents node, pick max
                weight[i][j] = lst[i][j] + max(weight[i-1][j-1], weight[i-1][j])
