...
def is_word_found(matrix, target):
    ...
    for i in range(no_rows):
        for j in range(no_cols):
            if (
                matrix[i][j] == first_target_char
            ):  # find the char in matrix matching with first target char
                row_string = "".join(
                    matrix[i][j:]
                )  # string from that char till end of that row
                col_string = "".join(
                    [row[j] for row in matrix[i:]]
                )  # string from that char till end of that column
                if (len(row_string) >= len_target) and (
                    row_string[:len_target] == target
                ):  # sucessfully find target in the row
                    return True
                if (len(col_string) >= len_target) and (
                    col_string[:len_target] == target
                ):  # successfully find target in the column
                    return True
