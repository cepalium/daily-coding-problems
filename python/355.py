...
def find_all_possible_rounded_arrays(X):
    ...
    Y_list = list(product(*rounded_options))  # treat all elements in rounded_options as *args for product()
...
def get_min_pairwise_absolute_difference_array(X, Y_list):
    ...
    min_arr = Y_list[0]  # initialized the min array with 1st element from all options
    for Y in Y_list[1:]:
