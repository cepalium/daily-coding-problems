    ...
    for i in range(n_rows):
        for j in range(n_cols):
            if (i == 0) and (j == 0):  # base case: position [0][0]
                ...
                sumCoins[i][j] = matrix[i][j] + max([sumCoins[i][j-1], sumCoins[i-1][j]])
    return sumCoins[n_rows-1][n_cols-1] # return no. coins at bottom right corner position
...
if __name__ == "__main__":
    maxNumberCoins_test(matrix=[[0,3,1,1],[2,0,0,4],[1,5,3,1]], desiredVal=12)    # print True