...
def k_most_similar_pair_websites(pairs, k):
    ...
    top_k = [
        (similarities[i][0], similarities[i][1]) for i in range(k)
    ]  # list of tuples (web_a, web_b)
    return top_k
...
def build_matrix(pairs):
    ...
    visit_matrix = pd.DataFrame(
        0, columns=websites, index=users
    )  # visit_matrix is full of 0
...
def build_similarity_list(matrix):
    ...
    for i in range(0, n - 1):
        for j in range(i + 1, n):
            sim = similarity(
                matrix[col[i]].to_numpy(), matrix[col[j]].to_numpy()
            )
            similarities.append((col[i], col[j], sim))
    similarities = sorted(
        similarities, key=lambda similarities: similarities[2], reverse=True
    )
    return similarities
...
def similarity(A, B):
    ...
    )  # max match number is the smaller length
    for i in range(n):
...
def test3():
    assert k_most_similar_pair_websites(pairs=pairs, k=2) == [
        ("a", "e"),
        ("a", "c"),
    ]
...
def test4():
    assert k_most_similar_pair_websites(pairs=pairs, k=3) == [
        ("a", "e"),
        ("a", "c"),
        ("c", "e"),
    ]
