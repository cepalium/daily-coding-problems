...
def superSet(arr):
    ...
    for i in range(
        2**len_arr
    ):  # number of subsets = 2^len(array), e.g array has 3 elements -> superset has 8=2^3 subsets
        bin_i = convertIntToFixedLengthBinary(i, len_arr)
        s = [
            arr[i] for i in range(len_arr) if bin_i[i] == "1"
        ]  # subset has all elements where "1" in bin_i
