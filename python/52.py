...
class LRUCache:
    def __init__(self, size):
        ...
        self.deq = (
            collections.deque()
        )  # init double-ended queue: store key ~> LRU element in last
    ...
    * If there are already n items in the cache and we are adding a new item, then it should also remove the least recently used item.
    ...
    def set(self, key, value):
        if len(self.deq) == self.size:  # if cache is full
            lru_key = (
                self.deq.pop()
            )  # get rid of the LRU element by popping the last element in deq
            del self.dict[lru_key]  # delete (key,value) in dict
        self.deq.appendleft(
            key
        )  # add key to front of deq, i.e most-recently used
...
def main():
    ...
    cache.set(
        4, 40
    )  # remove LRU element, i.e (1,10) ~> dict {2:20, 3:30, 4:40}
    cache.set(
        5, 50
    )  # remove LRU element, i.e (2,20) ~> dict {3:30, 4:40, 5:50}
