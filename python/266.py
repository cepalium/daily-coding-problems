...
def step_words(words, input_word):
    ...
    for word in words:
        if (
            len(input_word) - len(word) != 1
        ):  # current word cannot be added 1 letter to be a valid step word
            continue
        freq_input = letter_frequency(input_word)
        freq_cur = letter_frequency(word)
        diff = (
            0  # sum of different no. each letters in current word & input word
        )
        for c in freq_input.keys():
            diff += abs(freq_input[c] - freq_cur[c])
        if (
            diff == 1
        ):  # current word needs to add 1 letter to be valid step word
            valid.append(word)
    return valid
...
def test1():
    assert step_words(
        words=["apple", "pear", "lemon", "paple"], input_word="appeal"
    ) == ["apple", "paple"]


def test2():
    assert step_words(
        words=["a", "ab", "abc", "abcd", "abcde"], input_word="abcd"
    ) == ["abc"]
