...
# output: shortest substring s.t contains all chars
    ...
    if not string or not characters:
        return None
    # else
    dict_char_position = {c: [] for c in characters}    # dict {'char': [list to store index of char in string]} initialized
    ...
    products = itertools.product(*char_position_list)   # cartesian product, e.g [[1,2,3], [0]] ~> [[1,0], [2,0], [3,0]]
    ...
    for e in products:  # check all pairs of catersian products
        tmp_dist = max(e) - min(e)  # temporary distance from most-right index to most-left index
        if tmp_dist < dist: # update dist, min_pos, max_pos if smaller distance
            dist = tmp_dist
            min_pos = min(e)
            max_pos = max(e)
