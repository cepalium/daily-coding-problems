...
def minCoins(x, availableCoins):
    ...
    availableCoins = sorted(
        availableCoins, reverse=True
    )  # sort available coins descendingly
    minCoins = math.inf  # initialize min coins for exchange to plus infinitiv
    n = len(availableCoins)
    for i in range(
        n
    ):  # find min no. coins by dropping the highest-value coin after each iteration
        val = x  # var to temporarily store the input money amount
        noCoins = 0  # no. coins for exchage
        for j in range(
            i, n
        ):  # find min no. coins possible for this this set of available coins
            if val < availableCoins[j]:  # impossible to exchange
                break
            noCoins += (
                val // availableCoins[j]
            )  # max no. coins possible w/ current coin value
            val = val % availableCoins[j]  # remainer after exchanging
        if (
            noCoins < minCoins
        ):  # update new value of min coins if found a smaller no. coins to exchange
            minCoins = noCoins
