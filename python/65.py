...
def nextMove(matrix, cellState, curX, curY, curDir):
    ...
    if (curDir == "left") and (
        (curX == 0) or cellState[curY][curX - 1]
    ):  # change left to up
        nextDir = "up"
    if (curDir == "right") and (
        (curX == m - 1) or cellState[curY][curX + 1]
    ):  # change right to down
        nextDir = "down"
    if (curDir == "down") and (
        (curY == n - 1) or cellState[curY + 1][curX]
    ):  # change down to left
        nextDir = "left"
    if (curDir == "up") and (
        (curY == 0) or cellState[curY - 1][curX]
    ):  # change up to right
        nextDir = "right"
...
def printClockwiseSpiral(matrix):
    ...
    state = [
        [False for i in range(m)] for j in range(n)
    ]  # matrix to store cell state ~> True if visited, False if not visited
    # loop
    while counter < n * m:
        ...
        x, y, direction = nextMove(
            matrix, state, x, y, direction
        )  # get next info of x, y, direction
