...
class BinaryTree:
    ...
    def _subtree_path_sum(self, n):
        ...
        if parent is None:  # n is root
            ...
            n._sum = (
                parent._sum + n._element
            )  # node sum = parent sum + node element
        if self.is_leaf(n):
            self._sum_collection.append(
                n._sum
            )  # add leaf sum to sum collection
        else:
            for c in self.children(
                n
            ):  # else: find path sum of its children recursively
