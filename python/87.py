...
# output:
    ...
    for rule in rules:  # go through all rules
        ...
        d, loc, s = ruleAnalyzer[0], ruleAnalyzer[1], ruleAnalyzer[2]   # destination "d", location "loc", source "s" from current rule
        if s not in points.keys():      # new source
            points[s] = [0, 0]
        x_s, y_s = points[s][0], points[s][1]               # get coords of source
        delta_x, delta_y = delta[loc][0], delta[loc][1]     # locate the distance of destination from source by direction
        ...
        if d not in points.keys():  # new destination
            ...
            real_x_d, real_y_d = points[d][0], points[d][1] # get destination coords calculated from previous rules
            if (real_x_d != x_d) and (real_y_d != y_d): # coords from previous rule are different from calculation of current rule 
