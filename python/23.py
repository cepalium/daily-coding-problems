...
def minStepInBoards(board, start, end):
    ...
    no_cols = len(board[0])
    d = [
        [no_rows * no_cols for i in range(no_cols)] for j in range(no_rows)
    ...
    while eventList:
        ...
        if (
            (0 <= upRow < no_rows)
            and (d[curRow][curCol] + 1 < d[upRow][curCol])
            and not board[curRow][curCol]
        ):
        ...
        if (
            (0 <= downRow < no_rows)
            and (d[curRow][curCol] + 1 < d[downRow][curCol])
            and not board[curRow][curCol]
        ):
        ...
        if (
            (0 <= leftCol < no_cols)
            and (d[curRow][curCol] + 1 < d[curRow][leftCol])
            and not board[curRow][curCol]
        ):
        ...
        if (
            (0 <= rightCol < no_cols)
            and (d[curRow][curCol] + 1 < d[curRow][rightCol])
            and not board[curRow][curCol]
        ):
...
if __name__ == "__main__":
    ...
    minStepInBoards_test(
        ...
        (3, 0),
        (0, 0),
    )  # return "null", row[1] is all blocked
