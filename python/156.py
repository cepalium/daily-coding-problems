...
def smallest_number_squared_integers(n):
    ...
    * ensure to have result because having 1 as smallest unit & n is positive int
    """
    squared_integer_list = [
        i**2 for i in range(n // 2, 0, -1) if i ** 2 <= n
    ]  # list of squared integers up to n descendingly
    ...
    for j in range(m):  # each iteration drops a number in squared_integer_list
        number = possible_smallest(
            n, squared_integer_list, j
        )  # find the number of integers which sum to n
        if (smallest is None) or (
            number < smallest
        ):  # update smallest number if possible
            smallest = number
    return smallest
...
def possible_smallest(n, lst, marker):
    ...
    if n == 0:
        ...
        q = (
            n // lst[marker]
        )  # quotient: number of integers possible by this int
