...
def propagation_time(N, edges):
    ...
    propagate_dict = {i:[-1, -1] for i in range(N+1)}  # dict { node i: [shortest time for msg from node 0 to reach node i, predecessor] }
    propagate_dict[0] = [0, -1] # initialize at node 0
    # propagation
    for edge in edges:
        a, b, t = edge[0], edge[1], edge[2]
        t_a, t_b, = propagate_dict[a][0], propagate_dict[b][0]
        if t_b == -1 or t_b > t_a + t:  # node b isn't reached yet or reach time at node b isn't optimal
