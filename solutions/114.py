...
# running time: O(n)
    # instance variables
    marker = 0  # all the element before this index has been checked
    ...
    start = True if string[0] not in delim else False  # used to identify the order of merging
    ...
    while pivot < len(string):
        if (not delimiterFlag) and (string[pivot] not in delim):   # continue sequence of chars that makes up a word
            pivot += 1
        elif (not delimiterFlag) and (string[pivot] in delim): # end of current word, start of new delimiter
            words.append(string[marker:pivot])  # add new word into list words
            marker = pivot  # update marker
            delimiterFlag = True    # update flag
        elif (delimiterFlag) and (string[pivot] not in delim): # end of current delimiter, start of new word
            delimiters.append(string[marker:pivot]) # add new delimiter into list delimiters
            marker = pivot  # update marker
...
if __name__ == "__main__":
    reverseWords_test("hello/world:here", DELIMITERS)    # print "here/world:hello"
    reverseWords_test("hello/world:here/", DELIMITERS)    # print "here/world:hello/"
    reverseWords_test("hello//world:here", DELIMITERS)    # print "here//world:hello"